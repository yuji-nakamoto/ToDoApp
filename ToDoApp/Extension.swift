...
extension UIViewController {
    ...
    func selectColor() {
        if UserDefaults.standard.object(forKey: GREEN_COLOR) != nil {
            ...
            orangeColor()
        } else if defaults.object(forKey: SETTING_VC) != nil{
            darkColor2()
        } else {
            darkColor()
        }
    ...
    func orangeColor() {
        navigationController?.navigationBar.barTintColor = UIColor(named: O_ORANGE)
        navigationController?.navigationBar.titleTextAttributes
            = [.foregroundColor: UIColor.white as Any,]
        tabBarController?.tabBar.barTintColor = UIColor(named: O_ORANGE)
        tabBarController?.tabBar.tintColor = UIColor.white
        tabBarController?.tabBar.unselectedItemTintColor = UIColor.systemGray
    }
